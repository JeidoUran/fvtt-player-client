export async function mergeAppData(imported: Partial<AppConfig>): Promise<AppConfig> {
    ...
    const merged: AppConfig = {
      ...
      ignoreCertificateErrors: imported.ignoreCertificateErrors ?? existing.ignoreCertificateErrors,
...
export async function mergeThemeData(imported: Partial<ThemeConfig>): Promise<ThemeConfig> {
    // Load existing config
    ...
    const merged: ThemeConfig = {
      ...
      buttonColorHoverAlpha: imported.buttonColorHoverAlpha ?? existing.buttonColorHoverAlpha,
      ...
      particleOptions: {
        ...
        speedYMin: imported.particleOptions?.speedYMin ?? existing.particleOptions.speedYMin,
        speedYMax: imported.particleOptions?.speedYMax ?? existing.particleOptions.speedYMax,
